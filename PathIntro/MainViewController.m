...
- (void)setupView
{
    ...
    self.centerViewController = [storyboard instantiateViewControllerWithIdentifier:@"Center"];
...
{    
    ...
    if (_leftPanelViewController == nil)
    {
        ...
        self.leftPanelViewController = [storyboard instantiateViewControllerWithIdentifier:@"LeftView"];
...
-(void)movePanel:(id)sender
{
    ...
    if([(UIPanGestureRecognizer*)sender state] == UIGestureRecognizerStateChanged) {
        NSLog(@"enter change %f",translatedPoint.x);
        if(velocity.x > 0) {
            // NSLog(@"gesture went right");
        } else {
            // NSLog(@"gesture went left");
